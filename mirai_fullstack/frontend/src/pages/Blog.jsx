...
import { useBlogPosts, useBlogCategories } from '../api'
...
export default function Blog() {
  ...
  const { data, isLoading, isError, error, isFetching } = useBlogPosts(
    selectedCat ? { category: selectedCat } : {}
  )
  // FIX: unwrap paginated results
    ...
    <>
      ...
      <section className="py-24">
        <div className="max-w-7xl mx-auto px-6">
          ...
          {/* Slim loading bar — non-blocking */}
          {(isLoading || isFetching) && (
            <div style={{ height:3, borderRadius:99, background:'#F0EDEA', overflow:'hidden', marginBottom:32 }}>
              <div style={{
                height:'100%', width:'60%', borderRadius:99,
                background:`linear-gradient(90deg, ${B}, ${F})`,
          ...
          <div className={`transition-opacity duration-200 ${isFetching ? 'opacity-60' : 'opacity-100'}`}>
            {(() => {
              const displayPosts = (posts && posts.length > 0) ? posts : (!isLoading ? FALLBACK_POSTS : [])
              if (displayPosts.length === 0) return (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7">
                  {FALLBACK_POSTS.map(p => <BlogCard key={p.id} post={p}/>)}
                </div>
              )
              return (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7">
                  {displayPosts.map(p => <BlogCard key={p.id} post={p}/>)}
                </div>
              )
            })()}
          </div>
